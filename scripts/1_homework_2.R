...
filenames <- list.files(path = "./data/homeworkData/multipleData/",
                        full.names = TRUE)

largeDataFrame <- as.data.frame(matrix(ncol = 11, nrow = 0))
colnames(largeDataFrame) = c("session", "chamber", "number", "type", "section",
                          "Cosponsr", "IntrDate", "Title", "NameFull",
                          "major_topic_label", "party_name")


for (i in 1:length(filenames)) {
  
  cat("Currently working on dataset:", i, "\n", sep = " ")
  
  temp <- import(filenames[i], setclass = "data.frame")
  
  largeDataFrame <- rbind(largeDataFrame, temp)
