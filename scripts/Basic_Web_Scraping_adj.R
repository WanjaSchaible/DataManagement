...
write.table(x = page_content,
            ...
            row.names = FALSE,
            quote = FALSE,
            file = "data/Example_statMethodsPage.html")
...
url <- "https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=Johann+Schaible&btnG="
...
get_google_scholar_results <- function(string,
    ...
    cat("Looking for:", string, "\n", sep = " ")

    # make the input name all lowercase
    string <- tolower(string)

    # split the string on spaces
    # the [[1]] retruns the first argument of the list that is returned
    ...
    # Why does the foolowing code does not give me the exact text
    # numResults <- str_split(page, '<div class=\\"gs_ab_mdw\\">')[[1]][2]
    # rm(numResults)
...
get_google_scholar_results("Johann Schaible")

get_google_scholar_results("Ansgar Scherp")
